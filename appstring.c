...
char* app_string_strcasestr(char* haystack, char* needle) {
    ...
    char *haystack_dup = (char*)app_string_dupstr(haystack, haystack_len),
         *needle_dup = (char*)app_string_dupstr(needle, needle_len);
    app_string_stringtolower(&haystack_dup, haystack_len);
    app_string_stringtolower(&needle_dup, needle_len);
    char* match_substr = strstr(haystack_dup, needle_dup);
    if(match_substr) {
        match_substrlen = strlen(match_substr);
        match_substr = (char*)app_string_dupstr(match_substr, match_substrlen);
    }
...
char* app_string_getutf8_charunixlike(const char* utf8_string, size_t* index, size_t utf8_stringlen) {
    mbstate_t state = {0};
...
char* app_string_getutf8_charwindows(const char* utf8_string, size_t* index, size_t utf8_stringlen) {
    wchar_t wchar = L'\0';
    int num_bytes = 0;
    char* utf8_char = NULL;
    if(*index >= utf8_stringlen) {
...
char* app_string_getutf8_char(const char* utf8_string, size_t* index, size_t utf8_stringlen) {
    char* utf8_char = NULL;
    #ifdef _WIN32
    utf8_char = app_string_getutf8_charwindows(utf8_string, index, utf8_stringlen);
    #else
    utf8_char = app_string_getutf8_charunixlike(utf8_string, index, utf8_stringlen);
    #endif
    return utf8_char;
}
